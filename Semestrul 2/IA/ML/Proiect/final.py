...
# train_data = train_data / norm_train
train_data = torch.from_numpy(train_data)

val_data = [read_image(row['id']) for _,row in val_labels.iterrows()]
val_data = np.array(val_data)
# norm_val = np.sum(np.abs(val_data ** 2), axis=1, keepdims=True) + 10 ** -8
# val_data = val_data / norm_val
...
# criterion = torch.nn.binary_cross_entropy(weights)
...
checkpoint = torch.load('saves3/model_state3.pth')
print(checkpoint.keys())

model_key_map = {
    'conv1.weight' : 'conv1.weight',
    'conv1.bias' : 'conv1.bias',
    # 'conv12.weight' : 'conv12.weight',
    # 'conv12.bias' : 'conv12.bias',
    'conv2.weight' : 'conv2.weight',
    'conv2.bias' : 'conv2.bias',
    # 'conv22.weight' : 'conv22.weight',
    # 'conv22.bias' : 'conv22.bias',
    'conv3.weight' : 'conv3.weight',
    'conv3.bias' : 'conv3.bias',
    'conv4.weight' : 'conv4.weight',
    'conv4.bias' : 'conv4.bias',
    'conv5.weight' : 'conv5.weight',
    'conv5.bias' : 'conv5.bias',
...
for epoch in range(max_epochs):
    ...
    for i, (data, labels) in enumerate(train_loader):   # primeste un batch de 32 de imagini
        ...
        # dupa 20 de epoci salvez modelul
    # if epoch % 2 == 0:
    #     torch.save(model.state_dict(), 'saves/model_state' + str(nr_saves) + '.pth')
    #     nr_saves += 1
    # Validate the model
    with torch.no_grad():
        val_loss = 0
    ...
    if no_improvement == earl_count:
        ...
        torch.save(model.state_dict(), 'saves4/model_state' + str(nr_saves) + '.pth')
        nr_saves += 1
        break
    try:
...
torch.save(model.state_dict(), 'saves4/model_state_final' + str(nr_saves) + '.pth')
